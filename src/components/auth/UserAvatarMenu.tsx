...
export default function UserAvatarMenu({
  ...
  //  • 'bottom-end' (default): below the button, right-aligned — for a TOP-RIGHT
  //    avatar (navbar / empty-chat header). The old hardcoded 'left-full' opened
  //    the menu to the right of the button, pushing it off the screen edge.
  //  • 'side': out to the right of the button — for the LEFT vertical sidebar.
  placement = 'bottom-end',
}: {
  className?: string;
  placement?: 'bottom-end' | 'side';
}) {
  ...
  return (
    <Menu as="div" className={cn('relative', className)}>
      ...
      >
        <MenuItems
          className={cn(
            'absolute w-56 rounded-xl shadow-xl bg-light-primary dark:bg-dark-primary border border-light-200 dark:border-dark-200 z-50 p-1 focus:outline-none',
            placement === 'side'
